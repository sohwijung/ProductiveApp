...
class ExerciseStatsViewController: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        titleText.text = "Mood vs. Exercise"
        ...
        NetworkManager.getAllData(username: userDefaults.string(forKey: "username")!) { (responseData) in
            ...
            for element in responseData{
                ...
                if (element.mood_data > 0){
                    if !differentXValues.contains(element.fitness_data_duration){
                        differentXValues.append(element.fitness_data_duration)
                    }
                ...
                sum = sum + Double(element.fitness_data_duration)
                count = count + 1.0
                if (Int(element.fitness_data_duration) < self.minX){
                    self.minX = Int(element.fitness_data_duration)
                }
                if (Int(element.fitness_data_duration) > self.maxX){
                    self.maxX = Int(element.fitness_data_duration)
                }
                 self.data.append(CGPoint(x: Int(element.fitness_data_duration), y: Int(element.mood_data)))
                }
            }
        ...
        NetworkManager.getCorrelation(username: userDefaults.string(forKey: "username")!, metric: "Fitness") { (correlationStruct) in
            var strength = "blank"
            ...
            self.someText.text = "There is a \(strength) \(sign) corelation between exercise duration and mood."
...
//        chartView.headerView.titleLabel.textColor = .blue
    ...
    @objc func setupGraph(){
        ...
        horizontalAxis.append("\(Double(minX))m")
        for i in 1...5{
           horizontalAxis.append("\(String(format: "%.1f", (0.000000001 + Double(minX) + (Double(i)*spacing))))m")
        }
